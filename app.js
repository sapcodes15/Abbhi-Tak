...
// Get it free at https://newsapi.org
// =============================================
const API_KEY = '518f4b7c0bdb49db838baaca6040ac86';
...
async function fetchNews(category) {
  ...
  // Map tab categories to search keywords for Indian financial news
...
function displayNews(articles) {
  ...
  grid.innerHTML = articles.map((article, index) => {
    ...
    const imgHTML = article.urlToImage
      ? `<img class="news-img" src="${article.urlToImage}" alt="news image" onerror="this.outerHTML='<div class=\'news-img-placeholder\'>📰</div>'" />`
...
async function searchNews(query) {
  ...
  try {
    ...
    // Remove active tab highlight during search
...
function showError(message) {
  document.getElementById('news-grid').innerHTML = `
    <div class="error-box">
      ...
      <p>${message}</p>
      <p style="margin-top:8px; font-size:11px;">Make sure your API key is set in app.js</p>
    </div>
  `;
}
...
// =============================================
// MARKET TICKER (Yahoo Finance via proxy)
// =============================================
const MARKET_DATA = [
  { id: 'sensex', symbol: '^BSESN', name: 'SENSEX', valEl: 'sensex-val', chgEl: 'sensex-chg' },
  { id: 'nifty', symbol: '^NSEI', name: 'NIFTY 50', valEl: 'nifty-val', chgEl: 'nifty-chg' },
  { id: 'usdinr', symbol: 'INR=X', name: 'USD/INR', valEl: 'usdinr-val', chgEl: 'usdinr-chg' },
  { id: 'gold', symbol: 'GC=F', name: 'GOLD', valEl: 'gold-val', chgEl: 'gold-chg' },
];

// NOTE: Yahoo Finance blocks direct browser calls due to CORS.
// For now we show placeholder data. In Phase 2 (Day 2) you'll
// set up a small backend to proxy these requests.
...
// Remember user's choice across page refreshes
